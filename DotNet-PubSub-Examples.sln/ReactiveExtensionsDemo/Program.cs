...
var subB  = bus.Subscribe(
    order => Console.WriteLine($"Subscriber B: Order {order.OrderId} for {order.CustomerEmail} totaling {order.Total}"),
    ex => Console.WriteLine($"Subscriber B Error: {ex.Message}"),
    () => Console.WriteLine("Subscriber B Completed"));
